...
type ActivityItem = {
  ...
  category: PurchaseCategory; paymentChannel: PaymentChannel;
};
...
const FALLBACK_DISCOVERY_META: Record<string, DiscoveryMeta> = {
  ...
  "hsbc-liveplus": { annualFee: 999, minMonthlyIncome: 50000, goals: ["Simple cashback"] },
  "axis-atlas": { annualFee: 5000, minMonthlyIncome: 100000, goals: ["Travel rewards", "Premium benefits"] },
  "hdfc-regalia-gold": { annualFee: 2500, minMonthlyIncome: 100000, goals: ["Travel rewards", "Premium benefits"] },
  "hdfc-dcb-metal": { annualFee: 10000, minMonthlyIncome: 250000, goals: ["Travel rewards", "Premium benefits"] },
  "hdfc-infinia": { annualFee: 12500, minMonthlyIncome: 300000, goals: ["Travel rewards", "Premium benefits"] },
...
function activityFromDatabase(row: InteractionRow): ActivityItem {
  const context = savedPaymentContext(row.full_response);
  return {
    id: row.id,
...
export default function Home() {
  ...
  const upgradeEvaluations = useMemo(() => {
    ...
    return Object.entries(discoveryMeta)
      ...
      .map(([cardId, meta]) => {
        ...
        const annualIncremental = Math.round(monthlyGain * 12);
        return {
          card: candidate,
          annualIncremental,
          annualFee: meta.annualFee,
          netAnnualValue: netIncrementalValue(annualIncremental, meta.annualFee),
          reasonCategory: largestGap.category,
          incomeFit: incomeBandValue(profile.incomeBand) >= meta.minMonthlyIncome,
          feeFit: meta.annualFee <= annualFeeLimit(profile.feeComfort),
          goalFit: meta.goals.includes(profile.primaryGoal),
        };
      })
  ...
  useEffect(() => {
    ...
    if (!saved) return;
    try {
      const guest = JSON.parse(saved) as {
        walletIds?: string[];
        merchant?: string;
        amount?: string;
  ...
  useEffect(() => {
    if (signedIn) return;
    window.localStorage.setItem("cardsmart-guest-session", JSON.stringify({
      walletIds,
      merchant,
      amount,
      purchaseCategory,
  ...
  }, [walletIds, merchant, amount, purchaseCategory, paymentChannel, view, signedIn]);
  ...
  const deleteAccount = async () => {
    ...
    const confirmedDelete = window.confirm("Delete your CardSmart account and all saved wallet, profile and payment data? This cannot be undone.");
    if (!confirmedDelete) return;
    setAccountDeleting(true);
    setAccountError("");
    const { error } = await supabase.rpc("delete_cardsmart_account");
    if (error) {
      setAccountDeleting(false);
      setAccountError("Your account could not be deleted. Please try again.");
      return;
    }
    await signOut();
    setAccountDeleting(false);
    setAuthNotice("Your CardSmart account and saved data were deleted.");
  };
  ...
  useEffect(() => {
    ...
    const syncFromHash = () => {
      const nextView = routes[window.location.hash.replace("#/", "")];
  ...
  useEffect(() => {
    const route = view === "home" ? "choose" : view === "wallet" ? "cards" : view === "explore" ? "find" : view === "activity" ? "savings" : view === "profile" ? "account" : "result";
              ...
              <div className="public-landing">
                <section className="landing-section decision-section" aria-labelledby="decisions-title">
                  ...
                  <div className="decision-grid">
                    <article className="decision-card decision-card--dark">
                      <span className="decision-number">01</span>
                      <div className="decision-icon"><Icon name="spark" size={22}/></div>
                      <h3>Pick the best card before you pay</h3>
                      <p>Tell us the store and amount. We compare every card in your wallet for that exact payment.</p>
                      <div className="mini-answer">
                        <span>Best for Swiggy · ₹2,000</span>
                        <strong>HDFC Swiggy</strong>
                        <b>₹200 back</b>
                      </div>
                    </article>
                    ...
                    <article className="decision-card">
                      ...
                      <div className="decision-icon"><Icon name="gift" size={22}/></div>
                      <h3>See the value you would have missed</h3>
                      <p>Track the payments you checked and see how much extra you earned by choosing better.</p>
                      <div className="mini-saving">
                        <span>Extra rewards found</span>
                        <strong>₹1,840</strong>
                        <small>Across 12 checked payments</small>
                      </div>
                    </article>

                    <article className="decision-card decision-card--mint">
                      <span className="decision-number">03</span>
                      <div className="decision-icon"><Icon name="compass" size={22}/></div>
                      <h3>Know if another card is worth adding</h3>
                      <p>We calculate what a new card adds beyond your current wallet, after its annual fee.</p>
                      <div className="mini-worth">
                        <span>Incremental annual value</span>
                        <strong>+₹4,260</strong>
                        <small>after annual fee</small>
                      </div>
                    </article>
                  </div>
                </section>
                ...
                <section className="landing-section example-section" aria-labelledby="example-title">
                  <div className="example-copy">
                    <span className="landing-kicker">A real decision, made obvious</span>
                    <h2 id="example-title">The card that feels right is not always the card that pays best.</h2>
                    <p>For an eligible ₹2,000 Swiggy order, two popular cards can be ₹100 apart. CardSmart shows the winner and the reason, before you check out.</p>
                    <button className="landing-link" onClick={() => document.getElementById("payment-checker")?.scrollIntoView({ behavior: "smooth", block: "center" })}>
                      Try this comparison <Icon name="arrow" size={17}/>
                    </button>
                  </div>
                  <div className="example-comparison" aria-label="Illustrative reward comparison">
                    <div className="comparison-context"><span>Swiggy order</span><strong>₹2,000</strong></div>
                    <div className="example-winner-card">
                      <div><span className="example-rank">Best choice</span><strong>HDFC Swiggy</strong><small>10% eligible cashback</small></div>
                      <b>₹200</b>
                    </div>
                    <div className="example-option">
                      <div><strong>SBI Cashback</strong><small>5% eligible online cashback</small></div>
                      <b>₹100</b>
                    </div>
                    <div className="example-option">
                      <div><strong>1% base-rate card</strong><small>Standard reward rate</small></div>
                      <b>₹20</b>
                    </div>
                    <div className="example-difference"><Icon name="spark" size={17}/><strong>₹180 more</strong><span>than using a basic 1% card</span></div>
                  </div>
                </section>

                <section className="landing-section how-section" aria-labelledby="how-title">
                  <div className="landing-heading landing-heading--compact">
                    <span className="landing-kicker">How CardSmart works</span>
                    <h2 id="how-title">From “which card?” to one clear answer.</h2>
                  </div>
                  <div className="how-grid">
                    <article><span>1</span><div className="how-icon"><Icon name="wallet" size={23}/></div><h3>Add the cards you own</h3><p>Choose card names from our catalogue. We never ask for card numbers or bank access.</p></article>
                    <article><span>2</span><div className="how-icon"><Icon name="search" size={23}/></div><h3>Describe the payment</h3><p>Enter a merchant or purchase and amount. Category and payment route are usually detected for you.</p></article>
                    <article><span>3</span><div className="how-icon"><Icon name="check" size={23}/></div><h3>Use the ranked answer</h3><p>See expected reward, the next-best option, caps and the assumptions behind the result.</p></article>
                  </div>
                </section>

                <section className="landing-section worth-section" aria-labelledby="worth-title">
                  <div className="worth-visual" aria-label="Illustrative new card value calculation">
                    <div className="worth-window-head"><span/><span/><span/><b>Should you add another card?</b></div>
                    <div className="worth-window-body">
                      <span className="worth-label">Estimated extra rewards</span><strong>₹6,760</strong>
                      <div><span>Less annual fee</span><b>− ₹2,500</b></div>
                      <div className="worth-total"><span>Net value beyond your wallet</span><b>+ ₹4,260 / year</b></div>
                      <small>Illustrative result based on your spend profile</small>
                    </div>
                  </div>
                  <div className="worth-copy">
                    <span className="landing-kicker">A recommendation with a higher bar</span>
                    <h2 id="worth-title">Not “best card.” Best card for what you already spend.</h2>
                    <p>Most lists rank cards in isolation. CardSmart compares a new card against the rewards your existing wallet already earns, then subtracts the fee. If it adds no real value, we tell you not to get it.</p>
                    <button className="secondary-button landing-secondary" onClick={() => setView("explore")}>See if I need another card <Icon name="arrow" size={17}/></button>
                  </div>
                </section>

                <section className="landing-section trust-section" aria-labelledby="trust-title">
                  <div className="trust-intro">
                    <span className="landing-kicker">Designed to need less data</span>
                    <h2 id="trust-title">Useful without asking for your financial secrets.</h2>
                    <p>A card recommendation does not need your card number, CVV, OTP or bank login. So CardSmart does not ask for them.</p>
                  </div>
                  <div className="trust-grid">
                    <article><Icon name="shield" size={24}/><h3>No sensitive card details</h3><p>Add the card name only. Never enter the number printed on your card.</p></article>
                    <article><Icon name="user" size={24}/><h3>You decide what to save</h3><p>Try a payment as a guest. Create an account only when you want to keep your wallet and history.</p></article>
                    <article><Icon name="info" size={24}/><h3>Reason shown with every answer</h3><p>See the rate, cap adjustment, exclusions and assumptions used in the recommendation.</p></article>
                    <article><Icon name="close" size={24}/><h3>Delete account data</h3><p>Signed-in users can permanently remove their profile, wallet and saved activity from Account.</p></article>
                  </div>
                </section>

                <section className="landing-section faq-section" aria-labelledby="faq-title">
                  <div className="faq-heading"><span className="landing-kicker">Questions, answered</span><h2 id="faq-title">Before you add your cards.</h2></div>
                  <div className="faq-list">
                    <details><summary>Does CardSmart make the payment for me?<Icon name="plus" size={18}/></summary><p>No. CardSmart helps you choose which card to use. You still pay normally on the merchant’s app, website or card machine.</p></details>
                    <details><summary>Do I need to share my card number or bank login?<Icon name="plus" size={18}/></summary><p>No. Add only the names of the cards you own. CardSmart does not need your card number, CVV, OTP or bank credentials.</p></details>
                    <details><summary>How does CardSmart decide the winning card?<Icon name="plus" size={18}/></summary><p>It evaluates the payment against each card’s modelled merchant, category and channel rules, then adjusts for known caps and exclusions. The assumptions are shown with the answer.</p></details>
                    <details><summary>Will it recommend a new card just because it has a high reward rate?<Icon name="plus" size={18}/></summary><p>No. The Find a Card flow estimates the extra value beyond what your current cards already earn and subtracts the annual fee. A card must add net value to rank well.</p></details>
                  </div>
                </section>

                <section className="landing-final-cta">
                  <span className="landing-kicker">Make the next payment count</span>
                  <h2>Stop guessing at checkout.</h2>
                  <p>Add the cards you own and get a ranked answer for your next payment.</p>
                  <div>
                    <button className="final-primary" onClick={openWalletPicker}>Add my cards <Icon name="arrow" size={18}/></button>
                    <button className="final-secondary" onClick={() => document.getElementById("payment-checker")?.scrollIntoView({ behavior: "smooth", block: "center" })}>Try a payment first</button>
                  </div>
                  <small>No card number, CVV, OTP or bank access required.</small>
                </section>

                <footer className="landing-footer">
                  <button className="brand" onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })} aria-label="Back to top">
                    <span className="brand-mark"><span>C</span></span><span className="brand-copy"><strong>CardSmart</strong><small>Pick right. Pay smart.</small></span>
                  </button>
                  <p>Credit-card decisions, made clear.</p>
                  <span>© 2026 CardSmart</span>
                </footer>
              </div>
            )}
          </div>
              ...
              <>
                ...
                <section className="explore-layout">
                  ...
                  {exploreCalculated && upgradeResult ? (
                    <aside className="upgrade-preview"><div className="preview-badge"><Icon name="spark" size={14}/> {exploreMode === "compare" ? "Card check" : "Your best upgrade"}</div><span className="mini-label">{upgradeResult.incomeFit && upgradeResult.feeFit ? "Basic profile fit" : "Fit needs review"}</span><h2>{upgradeResult.card.bank} {upgradeResult.card.name}</h2><span className={`rule-confidence rule-confidence--${upgradeResult.card.rewardModel.confidence}`}>{confidenceLabel(upgradeResult.card.rewardModel.confidence)}</span><div className="annual-value"><strong>{upgradeResult.netAnnualValue >= 0 ? "+" : "−"}₹{Math.abs(upgradeResult.netAnnualValue).toLocaleString("en-IN")}</strong><span>estimated net extra value / year<br/>after ₹{upgradeResult.annualFee.toLocaleString("en-IN")} annual fee</span></div><div className="value-reason"><span>What it adds</span><p>It adds about ₹{upgradeResult.annualIncremental.toLocaleString("en-IN")} in yearly reward value over your current wallet, mainly on {upgradeResult.reasonCategory} spend.</p></div>{(!upgradeResult.incomeFit || !upgradeResult.feeFit) && <div className="assumption-line"><Icon name="info" size={15}/><span>{!upgradeResult.incomeFit ? "Your selected income range may not meet this card’s stated requirement. " : ""}{!upgradeResult.feeFit ? "Its annual fee is outside your selected comfort level." : ""}</span></div>}<div className="assumption-line"><Icon name="info" size={15}/><span>Final approval remains with the issuer. Reward rules and fees should be rechecked before applying.</span></div></aside>
                  ) : exploreCalculated ? (
                    <aside className="upgrade-preview upgrade-preview--locked"><div className="locked-icon"><Icon name="check" size={22}/></div><span className="mini-label">No positive net upgrade</span><h2>Don’t add a paid card yet.</h2><p>None of the modelled cards adds enough reward value over your current wallet after its annual fee and your selected fit filters.</p></aside>
                  ) : (
            ...
            <div className="profile-layout">
              ...
              <aside className="profile-summary-card">
                ...
                <div className="account-danger-zone">
                  ...
                  <button type="button" disabled={accountDeleting} onClick={() => void deleteAccount()}>{accountDeleting ? "Deleting…" : "Delete my CardSmart data"}</button>
                </div>
              </aside>
            </div>
          </div>
